...
def train_model(model, train_loader, valid_loader, criterion, optimizer, num_epochs, device, model_path):
...
def main():
    # Prepare Data
    file_path = '/home/tanmay.somkuwar/implementation/sherlock_holmes_stories.txt'
    model_path = 'lstm_english.pth'
    ...
    # Train and evaluate
    train_model(model, train_loader, valid_loader, criterion, optimizer, num_epochs, device, model_path)
...
def train_model(model, train_loader, valid_loader, criterion, optimizer, num_epochs, device, model_path):
    for epoch in range(num_epochs):
        ...
        print(f'Epoch [{epoch + 1}/{num_epochs}]')
        print(f'Train Loss: {train_loss:.4f}, Train Accuracy: {train_acc:.4f}, Train Perplexity: {train_perplexity:.4f}')
...
def evaluate_model(model, dataloader, criterion, device):
    ...
    total_loss /= len(dataloader)
    total_acc /= len(dataloader)
    perplexity = calculate_perplexity(torch.tensor(total_loss))
...
def train_model(model, train_loader, valid_loader, criterion, optimizer, num_epochs, device):
    ...
    # Save model weights
    torch.save(model.state_dict(), "gru_english.pt")
    print("Model weights saved as 'gru_english.pt'")
...
def evaluate_model(model, dataloader, criterion, device):
    ...
    total_loss /= len(dataloader)
    total_acc /= len(dataloader)
    perplexity = calculate_perplexity(total_loss)
    return total_loss, total_acc, perplexity
